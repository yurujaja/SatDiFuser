...
class EuroSAT:
    def __init__(self, cfg: Dict[str, Any], split: str, augment: bool = False):
        # first check dataset path
        ...
        self.dataset_path = os.path.join(geobench_path, 'classification_v1.0', 'm-eurosat')
    ...
    def __getitem__(self, index):
        ...
        # Resize to the resolution of DiffusionSat, Transform into [-1, 1] 
    ...
    @staticmethod
    def get_splits(cfg: Dict[str, Any]) -> Tuple['EuroSAT', 'EuroSAT', 'EuroSAT']:

        dataset_train = EuroSAT(cfg, split="train")
        dataset_val = EuroSAT(cfg, split="val")
        dataset_test = EuroSAT(cfg, split="test")
